...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        return ['emergencyJobs' => $emergencyJobs, 'normalJobs' => $noramlJobs, 'cuser' => $cuser, 'usertype' => $usertype];
    }
...
//            $jobs['total'] = $totaljobs;
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == config('customer.roleid')) {
            $cuser = $user;
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        foreach ($translators as $translator) {
            ...
            $status = SendSMSHelper::send(config('sms.number'), $translator->mobile, $message);
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($cuser && $cuser->user_type == config('superadmin.roleid')) {
            $allJobs = Job::query();
